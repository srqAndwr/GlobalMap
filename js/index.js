...
(function() {
	...
	var local = function(){
		...
		let region = [1,100,1000,5000,10000,50000];
		//中国疫情地图API
		let option1 = {
			title: {
				text: '',
			...
			/*legend: {
			 ...
			 visualMap: {
			   ...
			   type: 'piecewise',
			   pieces:[ 
						{ min: region[5], max: 100000, label: '', color: '#000000' },
						{ min: region[4], max: 49999, label: '', color: '#372a28' },
						{ min: region[3], max: 9990, label: '', color: '#4e160f' },
						{ min: region[2], max: 4990, label: '', color: '#974236' },
						{ min: region[1], max: 990, label: '', color: '#ee7263' },
						{ min: region[0], max: 90, label: '', color: '#f5bba7' },
			   ],
	...
	$.ajax({
		...
		success: function(data) {
			let res = data.data;
			//console.log(res)
			res = JSON.parse(res).areaTree;
			res = res[0]['children'];
			let chinaData = [];
			for(let privice of res){
				chinaData.push({
					"name":privice.name,
					"value":privice.total.confirm
				});
			}
				...
				itemStyle: {
					normal: {
						color: function(params) {
							var colorList = [
								"#a71a4f", "#c71b1b", "#d93824", "#e7741b", "#dc9e31", "#d2b130",
								"#8cc13f", "#53b440", "#48af54", "#479c7f", "#48a698", "#57868c"
							];
